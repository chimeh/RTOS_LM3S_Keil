...
void RIT128x96x4LineDraw(unsigned long ulX,
                    unsigned long ulY,
                    unsigned long ulLength,
                    unsigned char ucLevel,
                    unsigned char ucHorizontal)
{
    unsigned int i;
    //
    // Check the arguments.
...
//    ASSERT((ulWidth & 1) == 0);
    ...
    g_pucBuffer[1] = ulX / 2;
    if (ucHorizontal == 1) {
        g_pucBuffer[2] = (ulX + ulLength - 2) / 2;
    } else {
        g_pucBuffer[2] = (ulX + 2) / 2;
    }
    ...
    g_pucBuffer[1] = ulY;
    if (ucHorizontal == 0) {
        g_pucBuffer[2] = ulY + ulLength - 1;
    } else {
        g_pucBuffer[2] = ulY + 2;
    }
    RITWriteCommand(g_pucBuffer, 3);
    RITWriteCommand(g_pucRIT128x96x4HorizontalInc, sizeof(g_pucRIT128x96x4HorizontalInc));

    for (i = 0; i < 8; i++) {
        g_pucBuffer[i] = ((ucLevel << 4) & 0xf0) | ((ucLevel << 0) & 0x0f);
    }
    ...
    if (ucHorizontal == 1) {
        ...
        while (ulLength > 0) {
            if (ulLength >= 16) {
                ...
                RITWriteData(g_pucBuffer, ulLength);
                ulLength  = 0;
            }
        }
    }
